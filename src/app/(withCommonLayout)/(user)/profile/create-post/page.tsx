...
const PostCreationModal = ({ isVisible, onClose }: any) => {
  ...
  const handleEditorChange = (content: any, editor: any) => {
    ...
    let decodedContent = he.decode(htmlContent);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 font-titlefont">
      ...
      <div className="bg-white relative rounded-lg shadow-lg p-6 w-11/12 md:w-2/3 lg:w-1/2 max-h-[90vh] overflow-auto">
        <h2 className=" text-black text-xl font-bodyfont mb-4">
          Create Travel Post
        </h2>
        ...
        <div className="flex justify-end space-x-4 mt-4">
          <button
            className="px-3 py-2 text-sm  bg-designColor opacity-80 text-white rounded hover:opacity-100 duration-500"
            onClick={onClose}
          >
            Cancel
          </button>
          <button
            className="px-3 py-2 text-sm bg-blue-500 text-white rounded hover:bg-blue-700 duration-500"
